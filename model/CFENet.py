...
from model.channel_attn import get_corr_chanfeat
...
class OneModel(nn.Module):
    ...
    def forward(self, x, s_x, s_y, y_m, y_b, cat_idx=None, name_dict=None, epoch=None):   
        ...
        with torch.no_grad():
            ...
            # print("len(query_feats): ", len(query_feats))
            # for feat in query_feats:
            #     print("feat.shape: ", feat.shape)
            # for backbone_feat in query_backbone_layers:
            #     print("query_backbone_feat.shape: ", backbone_feat.shape)
        if self.vgg:
        ...
        corr_query_mask = torch.cat(corr_query_mask_list, 1)               # corr_query_mask.shape:torch.Size([10, 5, 60, 60])
        # print("weight_soft.shape:{}, corr_query_mask.shape:{}".format(weight_soft.shape, corr_query_mask.shape))
        corr_query_mask = (weight_soft * corr_query_mask).sum(1, True)
