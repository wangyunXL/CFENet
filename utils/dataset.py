            ...
            if set(label_class).issubset(set(sub_list)):            # 滤除新颖类别， 这避免了训练集和测试集中有对象类别交叉的问题。保证了在训练中的对象类别，不会出现在测试中。
                for c in label_class:   # if语句判断了该张掩码图的全部类别是否都在训练集类别中。即不包含新颖类别
                    if c in sub_list:
                        ...
                        if tmp_label.sum() >= 2 * 32 * 32:          # 计算类别对象的像素总数目， 大于一定的值才将对象加入标签列表中
...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None, use_split_coco=False, \
        ...
        fss_list_root = '/root/autodl-tmp/my_work/BAM/lists/{}/fss_list/{}/'.format(data_set, mode)
    ...
    def __getitem__(self, index):
        ...
        # print("image_path: ", image_path)    # image_path:  E:/WY/BAM_win\data/MSCOCO2014/train2014/COCO_train2014_000000499693.jpg
...
class BaseData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'val' and self.batch_size == 1:
            return image, label, raw_label              # raw_label： (366, 500)
        else:
            return image, label
