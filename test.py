...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/coco/coco_split0_resnet50.yaml', help='config file') # coco/coco_split0_resnet50.yaml
...
# ----------------------  VAL  ----------------------
    ...
    print('pIoU:  {}'.format(np.round(pIoU_array, 4)))       # 返回的是iou_class[1]
    print('-'*43)
...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b) in enumerate(val_loader):
            ...
            output = F.interpolate(output, size=target.size()[1:], mode='bilinear', align_corners=True)       # [bs, 2, 60, 60]
            meta_out = F.interpolate(meta_out, size=target.size()[1:], mode='bilinear', align_corners=True)   #  pascal: torch.Size([bs, 2, 60, 60])
            base_out = F.interpolate(base_out, size=target.size()[1:], mode='bilinear', align_corners=True)   # pascal: torch.Size([bs, 16, 60, 60])
            
            loss = criterion(output, target)

            output = output.max(1)[1]             # [bs, 60, 60]
            meta_out = meta_out.max(1)[1]         #  pascal: torch.Size([bs, 60, 60])  ----> Supp_seg_pred
            base_out = base_out.max(1)[1]         # pascal: torch.Size([bs, 60, 60])  ----> Query_seg_pred

            subcls = subcls[0].cpu().numpy()[0]
            print("subcls: ", subcls)

            intersection, union, new_target = intersectionAndUnionGPU(output, target, args.classes, args.ignore_label)  # 得出的结果均为有两个元素的tensor， 分别对应0和1像素的个数
            intersection, union, new_target = intersection.cpu().numpy(), union.cpu().numpy(), new_target.cpu().numpy()
            intersection_meter.update(intersection), union_meter.update(union), target_meter.update(new_target)
            class_intersection_meter[subcls] += intersection[1]                           # class_intersection_meter的长度为5或者20， 对应多个类别的交集像素个数
            ...
            intersection, union, new_target = intersectionAndUnionGPU(base_out, target_b, split_gap*3+1, args.ignore_label)  # 返回有16或者61个元素的tensor，对应不同类别的元素个数
            intersection, union, new_target = intersection.cpu().numpy(), union.cpu().numpy(), new_target.cpu().numpy()
            ...
            loss_meter.update(loss.item(), input.size(0))            # input.size(0): batch_size
    ...
    class_miou_m = 0
    class_miou_b = 0
    for i in range(len(class_intersection_meter)):        # 计算主损失 len(class_intersection_meter)=5或者20
    ...
    for i in range(len(class_intersection_meter_b)):      # len(class_intersection_meter)= 15或者60
    ...
    # 类别平均mIoU
    class_miou = class_miou*1.0 / len(class_intersection_meter)       # 数据被转化为float型，方便计算
    ...
    for i in range(split_gap):       # 5或者20
    ...
    for i in range(split_gap*3):       # 15或者60
