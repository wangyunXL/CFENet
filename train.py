...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='/root/autodl-tmp/my_work/BAM/config/pascal/1shot/pascal_split0_1shot_resnet101.yaml', help='config file') # coco/coco_split0_resnet50.yaml
...
# ----------------------  DATASET  ----------------------
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco':
        train_data = dataset.SemData(args, transform=train_transform, transform_tri=train_transform_tri, mode='train', \
    ...
    if args.evaluate:
        if args.resized_val:
            ...
            val_transform_tri = transform_tri.Compose([
                ...
                transform_tri.Normalize(mean=mean, std=std)])
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = dataset.SemData(args, transform=val_transform, transform_tri=val_transform_tri, mode='val', \
                                    use_split_coco=args.use_split_coco)                                   
...
# ----------------------  TRAINVAL  ----------------------
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate and epoch%1==0:   # return loss_meter.avg, mIoU, class_miou, class_miou_b, iou_class[1]
            loss_val, FBIoU, FBIoU_m, mIoU, mIoU_m, mIoU_b, pIoU = validate(val_loader, model, epoch, args)
            val_num += 1
            if main_process() and args.viz:
