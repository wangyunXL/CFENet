# CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.run --nproc_per_node=2 train_coco.py
import os
import datetime
import random
import time
...
from model import CFENet

from utils import our_dataset
from utils import transform, transform_tri, config
from utils.util import AverageMeter, poly_learning_rate, intersectionAndUnionGPU, get_model_para_number, setup_seed, get_logger, \
                                    fix_bn, check_makedirs, get_train_val_set

cv2.ocl.setUseOpenCL(False)
cv2.setNumThreads(0)

# CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 train_pascal.py 


"""
Number of Parameters: 57M
Number of Learnable Parameters: 4M
"""
id_to_name = {"0":"Background", "1":"Aero_plane", "2":"Bicycle", "3":"Bird", "4":"Boat", "5":"Bottle", "6":"Bus",   
                "7":"Car", "8":"Cat", "9":"Chair", "10":"Cow", "11":"Dining_table", "12":"Dog", "13":"Horse", 
                "14":"Motorbike", "15":"Person", "16":"Potted_plant", "17":"Sheep", "18":"Sofa", "19":"Train", "20":"Tv_monitor"} 


def get_parser():
    parser = argparse.ArgumentParser(description='PyTorch Few-Shot Semantic Segmentation')
    parser.add_argument('--arch', type=str, default='CFENet') 
    parser.add_argument('--viz', action='store_true', default=True)
    parser.add_argument('--config', type=str, default='/root/autodl-tmp/my_work/BAM/config/pascal/1shot/pascal_split2_1shot_resnet101.yaml', help='config file') # coco/coco_split0_resnet50.yaml
...
def get_save_path(args):
    ...
    args.snapshot_path = os.path.join("/root/autodl-tmp/my_work/CFENet", 'train_model/{}/{}/shot{}/split{}/{}/snapshot'
    .format(args.arch, args.data_set, args.shot, args.split, backbone_str))
    args.result_path = os.path.join("/root/autodl-tmp/my_work/CFENet", 'train_model/{}/{}/shot{}/split{}/{}/result'
    .format(args.arch, args.data_set, args.shot, args.split, backbone_str))
...
def get_model(args):
    ...
    if hasattr(model,'freeze_modules'):         # hasattr() 函数用于判断对象是否包含对应的属性。
        ...
        if os.path.isfile(resume_path):
            ...
            try: 
                ...
                for key in list(new_param.keys()):
                    new_param[key[14:]] = new_param.pop(key)
                model.load_state_dict(new_param)
...
# ----------------------  DATASET  ----------------------
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco':
        train_data = our_dataset.SemData(args, transform=train_transform, transform_tri=train_transform_tri, mode='train', \
                                        use_split_coco=args.use_split_coco)
    train_sampler = DistributedSampler(train_data, shuffle=True) if args.distributed else None
    ...
    if args.evaluate:
        if args.resized_val:
            ...
            val_transform_tri = transform_tri.Compose([
                ...
                transform_tri.Normalize(mean=mean, std=std)])
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = our_dataset.SemData(args, transform=val_transform, transform_tri=val_transform_tri, mode='val', \
                                    use_split_coco=args.use_split_coco)                                   
...
# ----------------------  TRAINVAL  ----------------------
    ...
    for epoch in range(args.start_epoch, args.epochs):
        if keep_epoch == args.stop_interval:       # 当epoch==stop_interval时，停止迭代
            break
        if args.fix_random_seed_val:
            setup_seed(args.manual_seed + epoch, args.seed_deterministic)        # 初始化固定的随机种子
        ...
        train_log = "split{}_train_log.txt".format(args.split)
        train_logpath = os.path.join(args.log_dir, "{}/{}/shot{}/split{}".format(args.arch, args.data_set, args.shot, args.split))
        check_makedirs(train_logpath)
        train_logpath = os.path.join(train_logpath, train_log)
        val_log = "split{}_val_epoch_log.txt".format(args.split)
        val_logpath = os.path.join(args.log_dir, "{}/{}/shot{}/split{}".format(args.arch, args.data_set, args.shot, args.split))
        check_makedirs(val_logpath)
        val_logpath = os.path.join(val_logpath, val_log)


        # ----------------------  TRAIN  ----------------------
        loss_train, mIoU_train, mAcc_train, allAcc_train = train(train_loader,
                                                     val_loader, model, optimizer, epoch, train_logpath, val_logpath)
        if main_process() and args.viz:
        ...
        if (epoch % args.save_freq == 0) and (epoch > 0) and main_process():
            filename = args.snapshot_path + '/epoch_{}_{:.4f}.pth'.format(epoch, mIoU_train)
            logger.info('Saving checkpoint to: ' + filename)
            if osp.exists(filename):
                os.remove(filename)            
            torch.save({'epoch': epoch, 'state_dict': model.module.state_dict(), 'optimizer': optimizer.state_dict()}, filename)


        # -----------------------  VAL  -----------------------
        if args.evaluate and epoch%1==0:   # return loss_meter.avg, mIoU, class_miou, class_miou_b, iou_class[1]
            loss_val, FBIoU, FBIoU_m, mIoU, mIoU_m, mIoU_b, pIoU = validate(val_loader, model, val_logpath, epoch, args)
            val_num += 1
            if main_process() and args.viz:
        ...
        # save model for <testing>
            if mIoU > best_miou:
                ...
                keep_epoch = 0
                if args.shot == 1:
                    filename = args.snapshot_path + '/train_epoch_' + str(best_epoch) + '_{:.4f}'.format(mIoU) + '_FBIoU_{:.4f}'.format(best_FBiou) + '.pth'
                else:
                    filename = args.snapshot_path + '/train{}_epoch_'.format(args.shot) + str(best_epoch) + '_{:.4f}'.format(mIoU) + '_FBIoU_{:.4f}'.format(best_FBiou) + '.pth'
                if main_process():
                    logger.info('Saving checkpoint to: ' + filename)
                    torch.save({'epoch': best_epoch, 'state_dict': model.module.state_dict(), 'optimizer': optimizer.state_dict()}, filename)
    ...
    if main_process():
        ...
        print('mIoU:{:.4f} \t  mIoU_b:{:.4f}'.format(best_miou, best_miou_b))
        print('FBIoU:{:.4f} \t pIoU:{:.4f}'.format(best_FBiou, best_piou))
        print('>'*80)
        print ('%s' % datetime.datetime.now())


def train(train_loader, val_loader, model, optimizer, epoch, train_logpath, val_logpath):
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls, name_dict) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss1, aux_loss2 = model(s_x=s_input, s_y=s_mask, x=input, y_m=target,                      \
                                                                    y_b=target_b, cat_idx=subcls[0], name_dict=name_dict, epoch=epoch)
        ...
        # save model for <testing>
            if mIoU > best_miou:
                ...
                keep_epoch = 0
                if args.shot == 1:
                    filename = args.snapshot_path + '/train_epoch_' + str(epoch-0.5) + '_{:.4f}'.format(mIoU) + '_FBIoU_{:.4f}'.format(best_FBiou) + '.pth'
                else:
                    filename = args.snapshot_path + '/train{}_epoch_'.format(args.shot) + str(epoch-0.5) + '_{:.4f}'.format(mIoU) + '_FBIoU_{:.4f}'.format(best_FBiou) + '.pth'
                if main_process():
                    logger.info('Saving checkpoint to: ' + filename)
    ...
    if main_process():
        ...
        train_log_txt_formatter = "{time_str}\t [Epoch] {epoch:03d}\t [Main_loss] {main_loss}\t [mIoU]{mIoU}\t [mAcc] {mAcc}\t             \
                                            [AllAcc] {allAcc} [IoU_class0]{iou_class0} [IoU_class1]{iou_class1}\n"
        log_write = train_log_txt_formatter.format(time_str=time.strftime("%Y_%m_%d_%H:%M:%S"),
                                        ...
                                        mAcc = mAcc,   
                                        allAcc = allAcc,
                                        iou_class0 = iou_class[0],
                                        iou_class1 = iou_class[1])
...
def validate(val_loader, model, val_logpath, epoch, args):
    if main_process():
        logger.info('>>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>')
    batch_time = AverageMeter()          # 批量时间
    model_time = AverageMeter()          # 模型时间
    data_time = AverageMeter()           #
    loss_meter = AverageMeter()          # 损失
    ...
    db_epoch = math.ceil(test_num/(len(val_loader)-args.batch_size_val))      # math.ceil(x)返回大于等于参数x的最小整数,即对浮点数向上取整
    iter_num = 0
    # print("db_epoch: ", db_epoch)

    with torch.no_grad():
        for e in range(db_epoch):
            for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b, name_dict) in enumerate(val_loader):
                ...
                output, meta_out, base_out = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b,
                                                            cat_idx=subcls, name_dict=name_dict, epoch=epoch)
                model_time.update(time.time() - start_time)

                if args.ori_resize:  # 将目标掩码转化为最长边大小的正方形，填充值为255
    ...
    iou_class = intersection_meter.sum / (union_meter.sum + 1e-10)          # （先计算各个类别的交集和并集的和）交并比
    ...
    if main_process():
        ...
        log_stats = {
        'epoch': epoch + 1,
        'FBIoU_f_mIoU': "{:.4f}".format(mIoU),
        'FBIoU_m_mIoU_m':  "{:.4f}".format(mIoU_m), 
        'mIoU_f_class_miou' : "{:.4f}".format(class_miou),   
        'mIoU_m_class_miou_m' : "{:.4f}".format(class_miou_m),
        'mIoU_b_class_miou_b' : "{:.4f}".format(class_miou_b),
        'IoU_class0': "{:.4f}".format(iou_class[0]),
        'IoU_class1': "{:.4f}".format(iou_class[1]),
        'IoU_m_class0': "{:.4f}".format(iou_class_m[0]),
        'IoU_m_class1': "{:.4f}".format(iou_class_m[1]),
        **{f'val_{k}': v for k, v in novel_dict.items()},
