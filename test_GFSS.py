...
def get_parser():
    ...
    parser.add_argument('--opts', help='see config/pascal/pascal_split0_vgg.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main():
    ...
    args = get_parser()                                       # 获取config
    logger = get_logger()                                     # 创建日志
    print(args)
    ...
    model = get_model(args)                                  # 获取模型
    logger.info(model)

# ----------------------  DATASET  ----------------------
    value_scale = 255                                        # 设置均值和方差
    mean = [0.485, 0.456, 0.406]
    mean = [item * value_scale for item in mean]
    std = [0.229, 0.224, 0.225]
    std = [item * value_scale for item in std]

    if args.resized_val:                                    # 设置图像变化
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco':                   # 加载数据
...
# ----------------------  VAL  ----------------------
    ...
    for val_id in range(val_num):         # val_num=5                         # 计算各项指标，验证5次
        val_seed = seed_array[val_id]       # (0,999)中随机取5个数
    ...
    total_time = time.time() - start_time                                    # 计算整个验证过程所用时间
    ...
    print('mIoU_n:  {}'.format(np.round(mIoU_array_n, 4)))                  # np.round(data, decimal)  decimal:保留的小数位数
...
def validate_GFSS(val_loader, model, val_seed):
    ...
    class_intersection_meter_t = [0]*split_gap*4        # 20或者80
    ...
    db_epoch = math.ceil(test_num/(len(val_loader)-args.batch_size_val))        # args.batch_size_val=1
    iter_num = 0
    novel_num = 0

    for e in range(db_epoch):           # 迭代开始
        for i, (input, target_t, s_input, s_mask, subcls, ori_label_t) in enumerate(val_loader):
            if iter_num * args.batch_size_val >= test_num:        # 判断迭代次数是否大于测试总次数
                break
            iter_num += 1
            data_time.update(time.time() - end)
            
            input = input.cuda(non_blocking=True)                # 将输入值复制到GPU中
            target_t = target_t.cuda(non_blocking=True)
            s_input = s_input.cuda(non_blocking=True)
            s_mask = s_mask.cuda(non_blocking=True)           
            ori_label_t = ori_label_t.cuda(non_blocking=True)

            novel_num +=1 if split_gap*3+1 in target_t.unique() else 0     # unique()：返回参数数组中所有不同的值，并按照从小到大排序   https://blog.csdn.net/weixin_39549734/article/details/81224567
            start_time = time.time()
            output, meta_out, base_out = model(x=input, s_x=s_input, s_y=s_mask, y_m=None, y_b=None, cat_idx=subcls)
            model_time.update(time.time() - start_time)           # 更新模型时间

            if args.ori_resize:                                   # 将target_t填充为方形， 尺寸取最大边
            ...
            output = F.interpolate(output, size=target_t.size()[1:], mode='bilinear', align_corners=True)      # 上采样预测结果
