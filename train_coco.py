...
# CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 train_coco.py > coco_5shot_resnet101_split1_log.txt



def denormalize(feat):
    orig_images = np.zeros_like(feat)
    orig_images[:, :, 0] = feat[:, :, 0] * 255.
    orig_images[:, :, 1] = feat[:, :, 1] * 255.
    orig_images[:, :, 2] = feat[:, :, 2] * 255.
    return orig_images
...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='/root/autodl-tmp/my_work/BAM/config/coco/5shot/coco_split3_5shot_resnet101.yaml', help='config file') # coco/coco_split0_resnet50.yaml
